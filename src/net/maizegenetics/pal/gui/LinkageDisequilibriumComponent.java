...
public class LinkageDisequilibriumComponent extends JComponent {
    ...
    public LinkageDisequilibriumComponent(LinkageDisequilibrium theLD, boolean includeBlockSchematic, boolean chromosomalScale, int windowSize, int windowX, int windowY) {
        ...
        this.diseq = new double[myWindowSize][myWindowSize];
        setXStart();
        setYStart();
    ...
    /**
     * This sets the X start position
     */
    ...
    /**
     * This sets the Y start position
     */
    ...
    private void addPolymorphismLabels(Graphics g, int ih) {
        ...
        for (int c = myYStart; c < myYEnd; c++) {
            if (jump[c] != 1) {
                String tempLocus = theAA.getLocusName(c+jump[c]);
                if ((tempLocus == null) || (tempLocus.length() == 0) || tempLocus.equalsIgnoreCase("unknown")) {
                    tempLocus = theAA.getSNPID(c + jump[c]);
                }
                if (tempLocus == null) {
                    s = "s" + theAA.getPositionInLocus(c+jump[c]);
                } else {
                    s = tempLocus + "s" + theAA.getPositionInLocus(c+jump[c]);
                }
            } else {
                s = "";
            }
