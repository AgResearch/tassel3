 ...
 * @author Fei Lu
 */
public class TagAgainstAnchorBiParental {
	int ancChrCnt=10;
	...
	public TagAgainstAnchorBiParental (String tagsByTaxaFileS, String anchorFileS, String pedigreeFileS, String outfileS) {
		...
		this.redirect(pedigreeFileS, "F");

		this.MTMapping(false);
	...
	public static void main(String[] args) {
		...
		new TagAgainstAnchorBiParental (tagsByTaxaFileS, anchorFileS, pedigreeFileS, outfileS);
	}
	...
	public void MTMapping(boolean ifMT) {
	...
	//chr, bestSite, refAlignment.getPositionInLocus(bestSite), bestP, countSig, (double)-1, bestNewTestFamilies.length, (double)this.getFamilyCode(bestNewTestFamilies)}
		...
		public void run() {
			...
			for (int i = 0; i < tbt.getTagCount(); i++) {
				...
				ArrayList segregationList = this.getSegregationFamilyGroups(i, minPresentFamilies, 10, pThresh);
				...
				for (int j = 0; j < minGnum; j++) { //it should be bestChrs.length, only caculate 2 loci for now
					ScanChromosome scanOnBestChrom = new ScanChromosome(tbt, i, theAnchorChrInBits[bestChrs[j]-1], segregationFamilyGroups[j], j, theResults, 0.00001, 1);
