...
public class TagsOnPhysicalMap extends AbstractTags {
    ...
    public static enum SAMFormat {BWA, BOWTIE2};  // Supported SAM formats (each program defines some custom features)
	...
	variantDef=new byte[maxVariants][rows];     // allele state - A, C, G, T or some indel definition  // maxVariants bytes
        for (int var=0; var<maxVariants; var++) {
            for (int tagIndex = 0; tagIndex < rows; tagIndex++) {
                variantPosOff[var][tagIndex] = byteMissing;
                variantDef[var][tagIndex] = byteMissing;
            }
        }
	dcoP=new byte[rows];
    ...
    void initPhysicalSort() {
        ...
        IntComparator compPos = new IntComparator() {
           public int compare(int a, int b) {
            ...
            if(chromosome[index1]>chromosome[index2]) return 1;
            if(startPosition[index1]<startPosition[index2]) return -1;
            if(startPosition[index1]>startPosition[index2]) return 1;
   ...
   public void printRows(int numRows, boolean requirePhysPosition, int printChr) {
       ...
       for(int i=0; outCount<numRows; i++) {
            int r=(byPosition)?indicesOfSortByPosition[i]:i;
            if((requirePhysPosition==true)&&(chromosome[r]!=printChr)) continue;
            System.out.println(printRow(r));
            outCount++;
        }
    }
    ...
    public void writeBinaryFile(File outFile, int minResolution, boolean requirePhysPosition,
        ...
        if(requirePhysPosition){ fw.writeInt(mappedTags()[0]);}  // the index 0 provides the number of tags with unique positions
        else{ fw.writeInt(tagNum); }
        fw.writeInt(tagLengthInLong);
        fw.writeInt(maxVariants);
        for (int row = 0; row < tagNum; row++) {
            if((requirePhysPosition==true)&&(chromosome[row]==Integer.MIN_VALUE)) continue;
            for (int j = 0; j < tagLengthInLong; j++) {
    ...
    public int[] mappedTags(){
        ...
        for (int row = 0; row < tagNum; row++) {
            if (chromosome[row]==Integer.MIN_VALUE) { 
                if (multimaps[row]>0) result[multi]++;  
            } else {  
                result[unique]++; 
            }
        }
   ...
   public void readTextFile(File inFile) {
	...
	try {
		...
		mapP[row]=parseByteWMissing(inputLine[c++]);
                if(row%1000000==0) System.out.println("Row Read:"+row);
	    }
	} catch (Exception e) {
...
//    }
    ...
    public byte getMultiMaps(int index)   {return multimaps[index];}
    public int  getChromosome(int index)  {return chromosome[index];}
    public int  getStartPosition(int index) {return startPosition[index];}
    public int  getEndPosition(int index) {return endPosition[index];}
    public byte getDivergence(int index)  {return divergence[index];}
    public byte getMapP(int index)        {return mapP[index];}
    public byte getDcoP(int index)        {return dcoP[index];}
    ...
    public void setMapP(int index, double mapP) {
        if(Double.isInfinite(mapP)) {this.mapP[index] = Byte.MAX_VALUE; return;}
        if(Double.isNaN(mapP)||(mapP<0)||(mapP>1)) {this.mapP[index] = Byte.MIN_VALUE; return;}
        if(mapP<1e-126) {this.mapP[index] = Byte.MAX_VALUE; return;}
        this.mapP[index] = (byte)(-Math.round(Math.log10(mapP)));
    }

    public int addVariant(int tagIndex, byte offset, byte base) {
        for(int i=0; i<maxVariants; i++) {
            if((variantPosOff[i][tagIndex]==byteMissing)&&(variantDef[i][tagIndex]==byteMissing)) {
	...
	System.out.println("Reading SAM format tag alignment from: " + inputFileName);
        ...
        try {
            ...
            for (int i= 0; i < nHeaderLines; i++) { br.readLine();} // Skip over the header
            for (tagIndex = 0; tagIndex < tagNum; tagIndex++) {
                inputStr = br.readLine();
                parseSAMAlignment(inputStr, tagIndex);
                if(tagIndex % 1000000 == 0){System.out.println("Read "+tagIndex+" tags.");}
            }
            br.close();
        } catch (Exception e) {
	...
	try {
	    ...
	    while((currLine=br.readLine())!=null){
                ...
                if(inputLine[0].contains("@")){
                    ...
                    if(inputLine[1].contains("bowtie2")) mySAMFormat = SAMFormat.BOWTIE2;
                    nHeaderLines++;
                }else{
                    String chr = inputLine[2];
                    if(!chrNames.contains(chr)){chrNames.add(chr); }
                    tagNum++;
                    if(tagNum % 1000000 == 0){System.out.println("Counted "+tagNum+" tags.");}
                }
            }
            br.close();
    ...
    private void recordLackOfSAMAlign(int tagIndex, String tagS, String tagName, String nullS, byte SAMStrand) {
        recordTagFromSAMAlignment(tagIndex, tagS, tagName, nullS, SAMStrand);
    ...
    private void recordTagFromSAMAlignment(int tagIndex, String tagS, String tagName, String nullS, byte strand) {
        if (strand == -1) tagS = BaseEncoder.getReverseComplement(tagS);
    ...
    public void swap(int index1, int index2) {
        ...
        int tb;
        tb=tagLength[index1];tagLength[index1]=tagLength[index2]; tagLength[index2]=(byte)tb;
        tb=multimaps[index1];multimaps[index1]=multimaps[index2]; multimaps[index2]=(byte)tb;
        tb=chromosome[index1];chromosome[index1]=chromosome[index2]; chromosome[index2]=tb;
        tb=strand[index1];strand[index1]=strand[index2]; strand[index2]=(byte)tb;
        int ti;
        ti=startPosition[index1];startPosition[index1]=startPosition[index2]; startPosition[index2]=ti;
        ti=endPosition[index1];endPosition[index1]=endPosition[index2]; endPosition[index2]=ti;
        tb=divergence[index1];divergence[index1]=divergence[index2]; divergence[index2]=(byte)tb;
        for (int j = 0; j < maxVariants; j++) {
            tb=variantPosOff[j][index1]; variantPosOff[j][index1]=variantPosOff[j][index2]; variantPosOff[j][index2]=(byte)tb;
            tb=variantDef[j][index1]; variantDef[j][index1]=variantDef[j][index2]; variantDef[j][index2]=(byte)tb;
        }
        tb=dcoP[index1];dcoP[index1]=dcoP[index2]; dcoP[index2]=(byte)tb;
        tb=mapP[index1];mapP[index1]=mapP[index2]; mapP[index2]=(byte)tb;
    }
    ...
    public Locus getLocus(int tagIndex){
        if(chromosome[tagIndex]==intMissing){return null;} //Return null for unmapped tags
    ...
    public byte getStrand(int tagIndex){return strand[tagIndex];}
...
//        int[] positions = new int[getTagCount()];
        for (int i = 0; i < getTagCount(); i++) {
            ...
            if(getChromosome(tagIndex)!=chromosome)continue;
            ...
            for (int varIndex = 0; varIndex < maxVariants; varIndex++) {
                ...
                if(offset==Byte.MIN_VALUE) continue;
    ...
    private static TagsOnPhysicalMap uniqueTags(String[] filenames){
        ...
        for(String name: filenames){
            ...
            tagNum+=file.tagNum;
            if(file.maxVariants > maxVariants) maxVariants=file.maxVariants;
            if(file.getTagSizeInLong() > tagLengthInLong) tagLengthInLong=file.getTagSizeInLong();
        }
            ...
            for (int inTag= 0; inTag < file.tagNum; inTag++) {  //Loop over tags in input
                ...
                for (int outVar= 0; outVar < output.maxVariants; outVar++) {  
                    ...
                    for (int inVar= 0; inVar < file.maxVariants; inVar++) {
                        ...
                        if(offset==byteMissing) continue;                            //Skip blank input variants
    ...
    public void filter(String[] filenames){
        ...
        for(String filename: filenames){
            ...
            hapmapSites.putAll(hapmapSites(ImportUtils.readFromHapmap(filename)));
        }
        ...
        if(topmSites.size() < hapmapSites.size()) System.out.println("Warning: more unique sites exist in hapmap file.");
...
//                System.out.print("Warning: SNP "+chr+":"+pos+" is not in TOPM.  ");
                ...
                for (int i= -basePerTag; i < basePerTag; i++){ 
                    if(fullSites.contains(i+pos)){ inRange=true; break; }}
...
//                    System.out.println("However, it is within range of a tag with the max. number of variants.");
        ...
        for(String site: skippedSites){System.out.println(site);}

        System.out.println("The following SNPs were not in the TOPM, and do not correspond to any known tag:");
        for(String site: insertedSites){System.out.println(site);}
        ...
        int removedSites=0;
           for (int tag = 0; tag < tagNum; tag++) {       
                ...
                for (int variant = 0; variant < maxVariants; variant++) {  
                    ...
                    if(!hapmapSites.containsKey(site)){ clearVariant(tag, variant);  removedSites++;  }
                }
            }
        ...
        if(topmSites.size() != hapmapSites.size()) System.out.println("Warning: number of filtered sites does not match number of alignment sites.");
    }
        ...
        HashSet<Integer> result= new HashSet<Integer>();
            for (int i= 0; i < tagNum; i++) {
                ...
                for (int j= 0; j < maxVariants; j++) {
                    ...
                    if (off == Byte.MIN_VALUE){ variantsFull=false; break;}
                }

                if(variantsFull)result.add(getStartPosition(i));
            }
        return result;
    }
    ...
    public static HashMap<String, Integer> hapmapSites(Alignment file) {
        ...
        for (int site = 0; site < file.getSiteCount(); site++) {        //Visit each site
            String pos=(
                    file.getLocusName(site) + "\t" +
                    file.getPositionInLocus(site));
            if(file.getPositionInLocus(site)>2000000000) System.out.println(pos);
            snps.put(pos, site);
        }
        return snps;
    }
