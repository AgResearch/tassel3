...
public class CompareGenosBetweenHapMapFilesPlugin extends AbstractPlugin {
    ...
    public static enum SiteCompareType {SAME_STRAND, EITHER_STRAND, DIFF_STRAND, DIFFERENT};
    ...
    public DataSet performFunction(DataSet input) {
        for (chr = startChr; chr <= endChr; chr++) {
            ...
            Alignment a1, a2;
            try {
                a1=ImportUtils.readFromHapmap(infile1);
            } catch(Exception e) {
                myLogger.info("Could not read the first input hapmap file for chr"+chr+":\n\t"+infile1+"\n\tSkipping...");
                continue;
            }
            try {
                a2=ImportUtils.readFromHapmap(infile2);
            } catch(Exception e) {
    ...
    private void findCommonPositionsAndCompare(Alignment a1, Alignment a2) {
        ...
        while (!finished) {
            while (a1.getPositionInLocus(s1) != a2.getPositionInLocus(s2) && s1<nSites1 && s2<nSites2) {
                while (a1.getPositionInLocus(s1) < a2.getPositionInLocus(s2) && s1<nSites1) s1++;
                while (a2.getPositionInLocus(s2) < a1.getPositionInLocus(s1) && s2<nSites2) s2++;
            }
    ...
    private double calculateF(Alignment a, int site) {
        ...
        byte hetGeno = IUPACNucleotides.getDegerateSNPByteFromTwoSNPs(majAllele, minAllele);
        int majGenoCnt=0, minGenoCnt=0, hetGenoCnt=0;
        int nTaxa = a.getSequenceCount();
        for (int taxon=0; taxon<nTaxa; taxon++) {
            byte base = a.getBase(taxon, site);
            if      (base == majAllele) majGenoCnt++;
            else if (base == minAllele) minGenoCnt++;
            else if (base == hetGeno)   hetGenoCnt++;
        }
    ...
    private int[] compareGenotypes(int site1, Alignment a1, int site2, Alignment a2, boolean sameStrand) {
        ...
        for (Integer taxon1Index : taxaRedirect.keySet()) {
            ...
            for (Integer taxon2Index : synTaxaIndicesForTaxonIndex) {
                ...
                byte base2 = sameStrand ? a2.getBase(taxon2Index, site2) : TagsToSNPByAlignmentPlugin.complement(a2.getBase(taxon2Index, site2));
                if (base1 != IUPACNucleotides.UNKNOWN_BYTE && base2 != IUPACNucleotides.UNKNOWN_BYTE) {
                    if ( !(nuc.isHeterozygote(base1) || nuc.isHeterozygote(base2)) ) {
                        if (base1!=base2) {
                            ++compareStats[nDiff];
                        ...
                        if (base1!=base2) ++compareStats[nDiff];
                    }
    ...
    public static SiteCompareType getSiteCompareType(byte[] alleles1, byte[] alleles2) {
        ...
        if (hetg1=='0'||hetg1=='-'||hetg1=='+'||hetg1==IUPACNucleotides.UNKNOWN_BYTE
          ||hetg2=='0'||hetg2=='-'||hetg2=='+'||hetg2==IUPACNucleotides.UNKNOWN_BYTE)
            return SiteCompareType.DIFFERENT; // indel sites not compared

        if (hetg1=='K' && hetg2=='K') return SiteCompareType.SAME_STRAND;  // GT v GT
        if (hetg1=='M' && hetg2=='M') return SiteCompareType.SAME_STRAND;  // AC v AC
        if (hetg1=='R' && hetg2=='R') return SiteCompareType.SAME_STRAND;  // AG v AG
        if (hetg1=='Y' && hetg2=='Y') return SiteCompareType.SAME_STRAND;  // CT v CT
        if (hetg1=='S' && hetg2=='S') return SiteCompareType.EITHER_STRAND;  // CG v CG
        if (hetg1=='W' && hetg2=='W') return SiteCompareType.EITHER_STRAND;  // AT v AT
        if (hetg1=='K' && hetg2=='M') return SiteCompareType.DIFF_STRAND;  // GT v AC
        if (hetg1=='M' && hetg2=='K') return SiteCompareType.DIFF_STRAND;  // AC v GT
        if (hetg1=='R' && hetg2=='Y') return SiteCompareType.DIFF_STRAND;  // AG v CT
        if (hetg1=='Y' && hetg2=='R') return SiteCompareType.DIFF_STRAND;  // CT v AG
        if (hetg1=='K' && hetg2=='R') return SiteCompareType.DIFFERENT;  // GT v AG
        if (hetg1=='K' && hetg2=='S') return SiteCompareType.DIFFERENT;  // GT v CG
        if (hetg1=='K' && hetg2=='W') return SiteCompareType.DIFFERENT;  // GT v AT
        if (hetg1=='K' && hetg2=='Y') return SiteCompareType.DIFFERENT;  // GT v CT
        if (hetg1=='M' && hetg2=='R') return SiteCompareType.DIFFERENT;  // AC v AG
        if (hetg1=='M' && hetg2=='S') return SiteCompareType.DIFFERENT;  // AC v CG
        if (hetg1=='M' && hetg2=='W') return SiteCompareType.DIFFERENT;  // AC v AT
        if (hetg1=='M' && hetg2=='Y') return SiteCompareType.DIFFERENT;  // AC v CT
        if (hetg1=='R' && hetg2=='K') return SiteCompareType.DIFFERENT;  // AG v GT
        if (hetg1=='R' && hetg2=='M') return SiteCompareType.DIFFERENT;  // AG v AC
        if (hetg1=='R' && hetg2=='S') return SiteCompareType.DIFFERENT;  // AG v CG
        if (hetg1=='R' && hetg2=='W') return SiteCompareType.DIFFERENT;  // AG v AT
        if (hetg1=='S' && hetg2=='K') return SiteCompareType.DIFFERENT;  // CG v GT
        if (hetg1=='S' && hetg2=='M') return SiteCompareType.DIFFERENT;  // CG v AC
        if (hetg1=='S' && hetg2=='R') return SiteCompareType.DIFFERENT;  // CG v AG
        if (hetg1=='S' && hetg2=='W') return SiteCompareType.DIFFERENT;  // CG v AT
        if (hetg1=='S' && hetg2=='Y') return SiteCompareType.DIFFERENT;  // CG v CT
        if (hetg1=='W' && hetg2=='K') return SiteCompareType.DIFFERENT;  // AT v GT
        if (hetg1=='W' && hetg2=='M') return SiteCompareType.DIFFERENT;  // AT v AC
        if (hetg1=='W' && hetg2=='R') return SiteCompareType.DIFFERENT;  // AT v AG
        if (hetg1=='W' && hetg2=='S') return SiteCompareType.DIFFERENT;  // AT v CG
        if (hetg1=='W' && hetg2=='Y') return SiteCompareType.DIFFERENT;  // AT v CT
        if (hetg1=='Y' && hetg2=='K') return SiteCompareType.DIFFERENT;  // CT v GT
        if (hetg1=='Y' && hetg2=='M') return SiteCompareType.DIFFERENT;  // CT v AC
        if (hetg1=='Y' && hetg2=='S') return SiteCompareType.DIFFERENT;  // CT v CG
        if (hetg1=='Y' && hetg2=='W') return SiteCompareType.DIFFERENT;  // CT v AT
