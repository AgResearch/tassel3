...
public class ReadBLASTer  {
    ...
    public ReadBLASTer(Tags refReads) {
        this.refTags=refReads;
    ...
    private void init() {
        int numOfLongsInRead=refTags.getTag(0).length;
        lookups=new int[2][2*refTags.getTag(0).length*refTags.getTagCount()];  //4 x 16base words
        int count=0;
        for(int i=0; i<refTags.getTagCount(); i++) {
            long[] cRead=refTags.getTag(i);
      ...
      //      System.out.println("word"+word+" string:"+BaseEncoder.getSequenceFromInt(word));
            ...
            while((hit<lookups[0].length)&&(lookups[0][hit]==word)) {
                ...
                if(!hitsAndDiv.containsKey(indexPossHapHit)) {
                    long[] possHitRead=refTags.getTag(indexPossHapHit);
