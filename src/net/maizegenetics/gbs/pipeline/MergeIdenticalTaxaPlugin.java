...
public class MergeIdenticalTaxaPlugin extends AbstractPlugin{
    ...
    private IUPACNucleotides theIUPAC=new IUPACNucleotides();
    ...
    public DataSet performFunction(DataSet input) {
        for (int chr = startChromosome; chr <= endChromosome; chr++) {
            ...
            MutableSimpleAlignment theMSA=new MutableSimpleAlignment(newGroup, a.getSiteCount(), a.getLoci());
            for (int s = 0; s < a.getSiteCount(); s++) {
                theMSA.setLocusOfSite(s, a.getLocusName(s));
            ...
            for (Map.Entry<String,List<String>> entry : sortedIds2.entrySet()) {
                ...
                if(entry.getValue().size()>1) {
                    ...
                    for (int s = 0; s < a.getSiteCount(); s++) {
                        ...
                        if(cb!=DataType.UNKNOWN_BYTE) known++;
                        if(theIUPAC.isHeterozygote(cb)) hets++;
                    }
            ...
            ExportUtils.writeToHapmap(theMSA, false, outfile, '\t');
        }
    ...
    private byte[] consensusCalls(Alignment a, List<String> taxa, boolean callhets, double majority) {
        ...
        for (int s = 0; s < a.getSiteCount(); s++) {
            ...
            if(totalCnt==0) continue;  //no data leave missing
            if((double)siteCnt[0][s]/(double)totalCnt>majority) {calls[s]=mj;}
            else if((double)siteCnt[1][s]/(double)totalCnt>majority) {calls[s]=mn;}
            else if(callhets) {calls[s]=het;}
        }
        return calls;
    }
    ...
    private void printUsage() {
        myLogger.info(
            "\n\n\nThe MergeIdenticalTaxaPlugin accepts the following options:\n"+
            ...
            "            specify multiple chromosome numbers.\n"+
            "-xHet     Exclude heterozygote calls (default: false)"+
    ...
    public void setParameters(String[] args) {
        if(args.length==0) {
            printUsage();
            throw new IllegalArgumentException("\n\nPlease use the above options (arguments).\n\n");
        }
