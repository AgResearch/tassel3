...
public class GBSHapMapFiltersPlugin extends AbstractPlugin{
    int startChromosome=1, endChromosome=1;
    ...
    public DataSet performFunction(DataSet input){
        for (int chr = startChromosome; chr <= endChromosome; chr++) {
            ...
            if(myArgsEngine.getBoolean("-mnTCov")) {
                ...
                if(lowCoverageTaxa==null) {lowCoverageTaxa=getLowCoverageLines(a, tCov);}  // Note: lowCoverageTaxa is based upon the startChromosome only
            ...
            if(hLD) {
                a=ImportUtils.readFromHapmap(outfile);
    ...
    private void printUsage() {
        myLogger.info(
            ...
            "-mnSCov  Minimum site coverage (default: no filter)\n"+
    ...
    public void setParameters(String[] args) {
        ...
        if (myArgsEngine.getBoolean("-mnSCov")) {minPresence = Double.parseDouble(myArgsEngine.getString("-mnSCov"));}
        ...
        if (myArgsEngine.getBoolean("-mxMAF")) {maxMAF = Double.parseDouble(myArgsEngine.getString("-mxMAF"));}
        if (myArgsEngine.getBoolean("-hLD")) {hLD = true;}
        if (myArgsEngine.getBoolean("-mnR2")) {
            if (hLD) minR2 = Double.parseDouble(myArgsEngine.getString("-mnR2"));
            else {
                printUsage();
        ...
        if (myArgsEngine.getBoolean("-mnBonP")) {
            if (hLD) minBonP = Double.parseDouble(myArgsEngine.getString("-mnBonP"));
            else {
                printUsage();
    ...
    public static String[] getLowCoverageLines(Alignment a, double pCoverage) {
        ...
        for (int i = 0; i < a.getSequenceCount(); i++) {
            ...
            for (int j = 0; j < a.getSiteCount(); j++) {
                if(a.getBase(i, j)!=DataType.UNKNOWN_BYTE) covered++;
            }
            double propCovered=(double)covered/(double)a.getSiteCount();
//            myLogger.info(a.getTaxaName(i)+":"+propCovered);
            if(propCovered<pCoverage) lowLines.add(a.getIdGroup().getIdentifier(i).getFullName());
        }
