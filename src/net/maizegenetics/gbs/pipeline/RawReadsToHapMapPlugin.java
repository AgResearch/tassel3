...
public class RawReadsToHapMapPlugin extends AbstractPlugin {
    ...
    private void printUsage() {
        myLogger.info(
            "\n\n\nThe options for the RawReadsToHapMapPlugin are as follows:\n"
                + "-i  Input directory containing fastq AND/OR qseq files\n"
                + "-k  Barcode key file\n"
                + "-e  Enzyme used to create the GBS library\n"
                + "-m  TagsOnPhysicalMap (TOPM) file containing tag genomic positions and variants (\"Production TOPM\")\n"
//                + "-d  Maximum divergence (edit distance) between new read and previously mapped read (Default: 0 = perfect matches only)\n"  // NOT IMPLEMENTED YET
                + "-o  Output directory to which HapMap genotype (.hmp.txt) files will be written\n\n\n"
        );
    }
    ...
    private void writeHapMapFiles(MutableSimpleAlignment[] outMSA, String outputDir, String[] rawSeqFileNames, int laneNum, int[] counters) {
        for (int i = 0; i < outMSA.length; i++) {
            ...
            outFileS = outFileS.replaceAll(rawSeqFileNameReplaceRegex,"_chr"+chromosomes[i]); // ".hmp.txt" gets added by ExportUtils.writeToHapmap
