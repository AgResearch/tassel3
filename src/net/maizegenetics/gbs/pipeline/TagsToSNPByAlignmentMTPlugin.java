...
//    static boolean ignoreTriallelic=false;
    ...
    public DataSet performFunction(DataSet input) {
        ...
        for(int i = startChr; i <= endChr; i++) {
             ...
             MutableSimpleAlignment theMSA=createMutableAlignment(theTBT, i, i, maxSize+100);
    ...
    public void runTagsToSNPByAlignment(TagsOnPhysicalMap theTOPM, TagsByTaxa theTBT, MutableSimpleAlignment theMSA,
...
//        System.out.println("Core Pool Size "+tpe.getCorePoolSize());
        ...
        for (int i = 0; (i < theTOPM.getSize())&&(theMSA.getNextFreeSite()<(maxSize-1000)); i++) {
            ...
            if(theTOPM.getChromosome(ri)!=targetChromo) continue;    //Skip tags from other chromosomes
...
//        System.out.println("TC:"+Thread.activeCount()+" BJC"+ConcurrencyTools.getThreadPool().getActiveCount());
    ...
    private int addTagToTreeMap(TreeMap<Integer, String> ts, int TagIndex, int[] longestSeqZeroDiv) {
        ...
        if(cnt>minTaxaCnt) {
            ...
            if(trimmedSeq.length()<64) trimmedSeq=trimmedSeq+polyN.substring(0, 64-trimmedSeq.length());
            if(trimmedSeq != null) {
    ...
    private static MutableSimpleAlignment createMutableAlignment(TagsByTaxa theTBT, int startChr, int endChr, int maxSites) {
...
//        System.out.println("TaxaRef:"+theMSA.getTaxaName(refGenTaxa)+" pos:"+startPos);
        for (int s = 0; s < tagAlignment.getSiteCount(); s++) {
            ...
            byte[] genotypes=null;
            if((genotypes=isSiteGood(calls,this.inclRare, this.inclGaps,this.minMAF,this.minMAC, this.minF))==null) continue;
            int currSite=theMSA.getNextFreeSite();
            theMSA.setLocusOfSite(currSite, ""+chromosome);
 ...
 //                   if(tx==refGenTaxa) System.out.printf("setnor pos: %d tx:%d s:%d b:%d %n", startPos, tx, currSite, complement(calls[tx]));
            ...
            if(this.isUpdateTOPM) updateTOPM(tagAlignment, s, position, strand, genotypes);
...
//            System.out.println(topmTagIndex+":"+tg+":"+theTOPM.printRow(topmTagIndex));
    ...
    private byte[] makeSNPCalls(TagsByTaxa theTBT, Alignment tagAlignment, int tagAlignSite) {
        ...
        for (int tg = 0; tg < tagAlignment.getSequenceCount(); tg++) {
            ...
            for (int tx = 0; tx < theTBT.getTaxaCount(); tx++) {
                if(theTBT.getReadCountForTagTaxon(tagIndex, tx)>0) {
                    ...
                    if(currentBase==Byte.MIN_VALUE) calls[tx]=baseToAdd;
        ...
        for (int tx = 0; tx < theTBT.getTaxaCount(); tx++) {
            if(calls[tx]==Byte.MIN_VALUE) calls[tx]=DataType.UNKNOWN_BYTE;
        }
        return calls;
    }
    ...
    private byte[] isSiteGood(byte[] calls, boolean includeRare, boolean includeGap, double minMAF, int minMAC, double minF) {
        ...
        if ( (theMAF<minMAF) && (alleles[1][1]<minMAC) ) return null;
        byte majAllele = (byte) alleles[0][0];
        byte minAllele = (byte) alleles[0][1];
        if ( !includeGap && ( (majAllele==DataType.GAP_BYTE) || (minAllele==DataType.GAP_BYTE) ) ) return null;
...
//        System.out.printf("%d %d %d propHets:%g expHets:%g obsF:%g %n", majGCnt, minGCnt, hetGCnt, propHets, expHets, obsF);
    ...
    private class SNPsFromSequenceAlignment extends Thread {
        ...
        public void run()  {  // Leave in the comments below
            ...
            if(tagSeqMap.size()>maxAlignmentSize) return;   // upper limit of tags
...
//            System.out.printf("FilteredAlign:%d%n%s%n", startPos,faa.toString());
            ...
            if(faa.getSiteCount()>maxSNPsPerLocus) return;
    ...
    public static byte resolveSNPByteFromCallPair(byte currCall, byte newCall) {
        ...
        if (newCall == 'A') {
            switch (currCall) {  // conflicts (more than 2 alleles) get set to N
                case 'A':  snpByte = 'A';  break;
                case 'C':  snpByte = 'M';  break;
                case 'G':  snpByte = 'R';  break;
                case 'T':  snpByte = 'W';  break;
                case 'K':  snpByte = 'N';  break;
                case 'M':  snpByte = 'M';  break;
                case 'R':  snpByte = 'R';  break;
                case 'S':  snpByte = 'N';  break;
                case 'W':  snpByte = 'W';  break;
                case 'Y':  snpByte = 'N';  break;
                case '-':  snpByte = '0';  break;
                case '+':  snpByte = 'N';  break; // it should not be possible for currCall to be '+'
                case '0':  snpByte = '0';  break;
                case 'N':  snpByte = 'N';  break; // was set to N because of a previous conflict, so should stay as N 
                default:   snpByte = 'N';  break;
            }
        } else if (newCall == 'C') {
            switch (currCall) {  // conflicts (more than 2 alleles) get set to N
                case 'A':  snpByte = 'M';  break;
                case 'C':  snpByte = 'C';  break;
                case 'G':  snpByte = 'S';  break;
                case 'T':  snpByte = 'Y';  break;
                case 'K':  snpByte = 'N';  break;
                case 'M':  snpByte = 'M';  break;
                case 'R':  snpByte = 'N';  break;
                case 'S':  snpByte = 'S';  break;
                case 'W':  snpByte = 'N';  break;
                case 'Y':  snpByte = 'Y';  break;
                case '-':  snpByte = '0';  break;
                case '+':  snpByte = 'N';  break; // it should not be possible for currCall to be '+'
                case '0':  snpByte = '0';  break;
                case 'N':  snpByte = 'N';  break; // was set to N because of a previous conflict, so should stay as N 
            ...
            switch (currCall) {  // conflicts (more than 2 alleles) get set to N
                case 'A':  snpByte = 'R';  break;
                case 'C':  snpByte = 'S';  break;
                case 'G':  snpByte = 'G';  break;
                case 'T':  snpByte = 'K';  break;
                case 'K':  snpByte = 'K';  break;
                case 'M':  snpByte = 'N';  break;
                case 'R':  snpByte = 'R';  break;
                case 'S':  snpByte = 'S';  break;
                case 'W':  snpByte = 'N';  break;
                case 'Y':  snpByte = 'N';  break;
            ...
            switch (currCall) {  // conflicts (more than 2 alleles) get set to N
                case 'A':  snpByte = 'W';  break;
                case 'C':  snpByte = 'Y';  break;
                case 'G':  snpByte = 'K';  break;
                case 'T':  snpByte = 'T';  break;
                case 'K':  snpByte = 'K';  break;
                case 'M':  snpByte = 'N';  break;
                case 'R':  snpByte = 'N';  break;
                case 'S':  snpByte = 'N';  break;
            ...
            switch (currCall) {  
                case 'A':  snpByte = '0';  break;
                case 'C':  snpByte = '0';  break;
                case 'G':  snpByte = '0';  break;
                case 'T':  snpByte = '0';  break;
                case 'K':  snpByte = 'N';  break;
                case 'M':  snpByte = 'N';  break;
                case 'R':  snpByte = 'N';  break;
                case 'S':  snpByte = 'N';  break;
