 ...
 * @author jcg233
 */
public class RawReadsToHapMapQuantPlugin extends AbstractPlugin {
    private final Logger myLogger = Logger.getLogger(RawReadsToHapMapQuantPlugin.class);
    ...
    private HashMap<String,Integer> taxaNameIndices = null;
    private byte[] byteAlleleToIndex = null;
    private byte[] indexToByteAllele = null;
    private final static double errorRate = 0.01;  // NB: should be dividing this by 3
    ...
    public RawReadsToHapMapQuantPlugin() {
        super(null, false);
    }

    public RawReadsToHapMapQuantPlugin(Frame parentFrame) {
...
//                + "-d  Maximum divergence (edit distance) between new read and previously mapped read (Default: 0 = perfect matches only)\n");  // NOT IMPLEMENTED YET
    ...
    public void translateRawReadsToHapmap(String[] rawSeqFileNames, String keyFileS, String enzyme, String outputDir, TagsOnPhysicalMap theTOPM, int maxDiv) {
        for(int laneNum=0; laneNum<rawSeqFileNames.length; laneNum++) {
            ...
            MutableSimpleAlignment[] outMSA = setUpMutableSimpleAlignments(thePBR, theTOPM);
            ArrayList[][] alleleList = new ArrayList[outMSA.length][outMSA[0].getSequenceCount()];
            for (int chr=0; chr<outMSA.length; chr++) {
                for (int taxon=0; taxon<outMSA[0].getSequenceCount(); taxon++) {
                    ArrayList<String> nullArray = new ArrayList<String>();
                    alleleList[chr][taxon] = nullArray;
                }
            }
//            short[][][][] alleleCounts = setUpAlleleCounts(outMSA);
            ...
            try {
                while ((temp = br.readLine()) != null) {
                    ...
                    if (rr != null) {
                        ...
                        if (tagIndex < 0 && maxDiv > 0) tagIndex = findBestImperfectMatch(theTOPM, rr.getRead(), counters, maxDiv);
                        if (tagIndex < 0) continue;
                        counters[2]++;  // goodMatched++;
                        recordVariantsFromTag(alleleList, theTOPM, outMSA, tagIndex, taxaNameIndices.get(rr.getTaxonName()));
                    }
                }
                br.close();
            } catch(Exception e) {
            ...
            callSNPsQuantitatively(alleleList, outMSA);
    ...
    private ParseBarcodeRead setUpBarcodes(int laneNum, String[] rawSeqFileNames, String keyFileS, String enzyme) {
        ...
        fastq = true;
        if(rawSeqFileNames[laneNum].substring(rawSeqFileNames[laneNum].lastIndexOf(File.separator)).contains("qseq")) fastq=false;
        if (fastq) { System.out.println("\tThis file is assumed to be in fastq format");
        } else { System.out.println("\tThis file contains 'qseq' in its name so is assumed to be in qseq format"); }
        ...
        else {printFileNameConventions(rawSeqFileNames[laneNum]);  return thePBR;}
        System.out.println("Total barcodes found in key file for this lane:"+thePBR.getBarCodeCount());
        return thePBR;
    }

    private MutableSimpleAlignment[] setUpMutableSimpleAlignments(ParseBarcodeRead thePBR, TagsOnPhysicalMap theTOPM) {
        byteAlleleToIndex = new byte[Byte.MAX_VALUE];
        byteAlleleToIndex[65] = 0;  // A (byte 65)
        byteAlleleToIndex[67] = 1;  // C (byte 67)
        byteAlleleToIndex[71] = 2;  // G (byte 71)
        byteAlleleToIndex[84] = 3;  // T (byte 84)
        byteAlleleToIndex[45] = 4;  // - (byte 45)

        indexToByteAllele = new byte[5];
        indexToByteAllele[0] = 65;  // A (byte 65)
        indexToByteAllele[1] = 67;  // C (byte 67)
        indexToByteAllele[2] = 71;  // G (byte 71)
        indexToByteAllele[3] = 84;  // T (byte 84)
        indexToByteAllele[4] = 45;  // - (byte 45)
        ...
        for (int i = 0; i < outMSA.length; i++) {
            outMSA[i]=new MutableSimpleAlignment(thePBR.getTaxaNames(), uniquePositions.get(i).length, new Locus[] {loci[i]} );
        }
        taxaNameIndices = outMSA[0].taxonMap(); //Find the indices of taxa names within the MSA for quick lookup
    ...
    private short[][][][] setUpAlleleCounts(MutableSimpleAlignment[] outMSA) {
        myLogger.info("\nSetting up a multidimensional array to hold the allele counts...");

        short[][][][] alleleCounts = new short[5][outMSA[0].getSequenceCount()][outMSA.length][]; //[allele][taxon][chr][site]
        for (int allele = 0; allele <5; allele++) {
            for (int taxon = 0; taxon < outMSA[0].getSequenceCount(); taxon++) {
                for (int chrIndex = 0; chrIndex < outMSA.length; chrIndex++) {
                    alleleCounts[allele][taxon][chrIndex] = new short[outMSA[chrIndex].getSiteCount()];
                }
            }
        }
        return alleleCounts;
    ...
    private int findBestImperfectMatch(TagsOnPhysicalMap theTOPM, long[] read, int[] counters, int maxDiv) {
        ...
        if (bestHitsAndDiv.size() > 0) {
            ...
            if(bestHitsAndDiv.size()==1) counters[5]++; // singleImperfectMatches
            tagIndex = bestHitsAndDiv.firstKey();  // a random tag (firstKey) chosen to resolve the tie = suboptimal behavior
        }
        return tagIndex;
    }

    private void recordVariantsFromTag(TagsOnPhysicalMap theTOPM, MutableSimpleAlignment[] outMSA, int tagIndex, int taxonIndex) {
        ...
        for (int variant = 0; variant < theTOPM.maxVariants; variant++){
            ...
            if( (currBase==theTOPM.byteMissing) || (currBase==DataType.UNKNOWN_BYTE) ) continue;
    ...
    private void recordVariantsFromTag(short[][][][] alleleCounts, TagsOnPhysicalMap theTOPM, MutableSimpleAlignment[] outMSA, int tagIndex, int taxonIndex) {
        int chromosome = theTOPM.getChromosome(tagIndex);
        if(chromosome== Integer.MIN_VALUE) {
            return;
        }
        ...
        for (int variant = 0; variant < theTOPM.maxVariants; variant++){
            ...
            if( (base==theTOPM.byteMissing) || (base==DataType.UNKNOWN_BYTE) ) continue;
            int offset= theTOPM.getVariantPosOff(tagIndex, variant);
            int pos=startPos+offset;
            int currSite=outMSA[chrIndex].getSiteOfPhysicalPosition(pos, locus);
            if(currSite<0) continue;
            alleleCounts[byteAlleleToIndex[base]][taxonIndex][chrIndex][currSite]++;
        }
    }

    private void recordVariantsFromTag(ArrayList<String>[][] alleleList, TagsOnPhysicalMap theTOPM, MutableSimpleAlignment[] outMSA, int tagIndex, int taxonIndex) {
        int chromosome = theTOPM.getChromosome(tagIndex);
        if(chromosome== Integer.MIN_VALUE) {
            return;
        }
        ...
        for (int variant = 0; variant < theTOPM.maxVariants; variant++){
            ...
            int currSite=outMSA[chrIndex].getSiteOfPhysicalPosition(pos, locus);
            if(currSite<0) continue;
            alleleList[chrIndex][taxonIndex].add(currSite+"_"+base);
        }
    }

    private void callSNPsQuantitatively(short[][][][] alleleCounts, MutableSimpleAlignment[] outMSA) {
        myLogger.info("\nPerforming quantitative SNP calling...");
        for (int taxon = 0; taxon < alleleCounts[0].length; taxon++) {
            for (int chrIndex = 0; chrIndex < alleleCounts[0][taxon].length; chrIndex++) {
                for (int site = 0; site < alleleCounts[0][taxon][chrIndex].length; site++) {
                    short[] alleleCountsTaxonSite = new short[5];
                    for (int allele = 0; allele < 5; allele++) {
                        alleleCountsTaxonSite[allele] = alleleCounts[allele][taxon][chrIndex][site];
                    }
                    outMSA[chrIndex].setBase(taxon, site, resolveQuantGeno(alleleCountsTaxonSite));
                }
            }
        }
    }

    private void callSNPsQuantitatively(ArrayList<String>[][] alleleList, MutableSimpleAlignment[] outMSA) {
        myLogger.info("\nPerforming quantitative SNP calling...");
        int nSites=0;
        for (int chrIndex = 0; chrIndex < alleleList.length; chrIndex++) {
            for (int taxon = 0; taxon < alleleList[chrIndex].length; taxon++) {
                String[] siteAlleles = new String[10];
                alleleList[chrIndex][taxon].toArray(siteAlleles);
                Arrays.sort(siteAlleles);
                short[] alleleCountsTaxonSite = null;
                int currSite = Integer.MIN_VALUE;
                for (int record=0; record<siteAlleles.length; record++) {
                    String[] parse = siteAlleles[record].split("_");
                    int site = Integer.parseInt(parse[0]);
                    byte base = Byte.parseByte(parse[1]);
                    if (site == currSite) {  // collect the allele call from the site
                        alleleCountsTaxonSite[byteAlleleToIndex[base]]++;
                    } else {  // finish off the previous site and start a new one
                        if (currSite > -1) {
                            outMSA[chrIndex].setBase(taxon, currSite, resolveQuantGeno(alleleCountsTaxonSite));
                            ++nSites;
                        }
                        alleleCountsTaxonSite = new short[5];
                        alleleCountsTaxonSite[byteAlleleToIndex[base]]++;
                        currSite = site;
                    }
                }
                // finish the final site
                outMSA[chrIndex].setBase(taxon, currSite, resolveQuantGeno(alleleCountsTaxonSite));
                ++nSites;
            }
        }
        myLogger.info("...quantitative SNP calling performed for "+nSites+" sites\n");
    }

    private byte resolveQuantGeno(short[] alleleCounts) {
        short[][] sortedAlleleCounts = sortAllelesByCount(alleleCounts);
        short a1Count = sortedAlleleCounts[1][0];
        if (a1Count==0) {
            return DataType.UNKNOWN_BYTE;
        }
        short a2Count = sortedAlleleCounts[1][1];  // What if a3Count = a2Count? -- this situation is not dealt with
        byte a1 = indexToByteAllele[(byte) sortedAlleleCounts[0][0]];
        if (a2Count == 0) {
            return a1;
        }
        byte a2 = indexToByteAllele[(byte) sortedAlleleCounts[0][1]];
    ...
    private short[][] sortAllelesByCount(short[] alleleCounts) {
        short[][] result = new short[2][indexToByteAllele.length]; // result[0][i]=alleleIndex; result[1][i]=count
        for(short i=0; i<indexToByteAllele.length; i++) {
            result[0][i]= i;
            result[1][i]= alleleCounts[i];
        }
        boolean change = true;
        while (change) { // sort the alleles by descending frequency
            change = false;
            for (int k = 0; k < indexToByteAllele.length-1; k++) {
                if (result[1][k] < result[1][k + 1]) {
                    short temp = result[0][k];
                    result[0][k] = result[0][k + 1];
                    result[0][k + 1] = temp;
                    short tempCount = result[1][k];
    ...
    private void writeHapMapFiles(MutableSimpleAlignment[] outMSA, String outputDir, String[] rawSeqFileNames, int laneNum, int[] counters) {
        myLogger.info("\nWriting genotypes to output HapMap files...");
        for (int i = 0; i < outMSA.length; i++) {
            outMSA[i].sortSiteByPhysicalPosition();
