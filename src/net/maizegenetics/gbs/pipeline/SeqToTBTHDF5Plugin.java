...
public class SeqToTBTHDF5Plugin extends AbstractPlugin {
    ...
    public DataSet performFunction(DataSet input) {
        ...
        if(possibleFile.exists()==false) {TagsByTaxaByteHDF5TaxaGroups theTBT = new TagsByTaxaByteHDF5TaxaGroups(myMasterTags, myOutputTBTHDF5);}
...
//            int goodBarcodedReads = 0, allReads = 0, goodMatched = 0;
            ...
            if(fastqFileS[laneNum].substring(fastqFileS[laneNum].lastIndexOf(File.separator)).contains("qseq")) isFastQ=false;
            ...
            if(thePBR==null) continue;  //if no barcodes found move on
...
//            }
    ...
    private static byte[] getTagsDistribution(int tagNumber, IntArrayList tagDist) {
        ...
        for (int i : tagDist.elements()) {
           if(result[i]<Byte.MAX_VALUE) result[i]++; 
        }
        return result;
    }
