...
import net.maizegenetics.gbs.pav.TagPMap;
 ...
 * @author Fei Lu
 */
public class TagAgainstAnchorLongTimePosBlock {
	int ancChrCnt=10;
	int threadNumPerCore = 32; //key to performance, 32 or 64 tasks have the best performance, almost linear to the number of cores.
	int threadNum;
	File[] TBTFiles;
	...
	public TagAgainstAnchorLongTimePosBlock (String tagsByTaxaDirS, String anchorFileS, String tagPMapFileS, String outfileDirS) {
	...
	private void loopMapping (String outfileDirS) {
		...
		for (int i = 0; i < TBTFiles.length; i++) {
			...
			this.MTmapping();
			String outfileS = (outfileDirS + "/" + TBTFiles[i].getName()).replaceAll("tbt.byte", "mapping.txt");
			this.writeMappingResult(outfileS);
		}
	}
    
    private void loadTagPMap (String tagPMapFileS) {
        tmp = new TagPMap(tagPMapFileS);
    }
	...
	class Task implements Runnable {
		...
		public void run() {
			...
			for (int i = 0; i < tbt.getTagCount(); i++) {
                ...
                long[] testTagDist;
                int index = tmp.getTagIndex(testTag);
                if (index >= 0) {
                    blastChr = tmp.getBestChr(index);
                    blastPos = tmp.getBestPos(index);
                }
                ...
                for (int cn=0; cn<theAnchorChrInBits.length; cn++) {
                    ...
                    int chr=Integer.parseInt(theAnchorChrInBits[cn].getLocus(0).getChromosomeName());
                    if (chr == blastChr) {
                        scanOnChr[cn] = new ScanChromosomeMTR(theAnchorChrInBits[cn],testTagDist, cn, theResults,0.000001,1, blastPos);
                    }
                    else {
                        scanOnChr[cn] = new ScanChromosomeMTR(theAnchorChrInBits[cn],testTagDist, cn, theResults,0.000001,1, -1);
                    }
                    scanOnChr[cn].scan();
                }
                int countRealSig=0;
                ...
                int chr=Integer.parseInt(theAnchorChrInBits[bestAlignment].getLocus(0).getChromosomeName());
                ScanChromosomeMTR bestChrNewThres;
                if (chr == blastChr) {
                    bestChrNewThres = new ScanChromosomeMTR(theAnchorChrInBits[bestAlignment],testTagDist, 0, bestResWithNewThreshold,pRank[1],1, blastPos);       
                }
                else {
                    bestChrNewThres = new ScanChromosomeMTR(theAnchorChrInBits[bestAlignment],testTagDist, 0, bestResWithNewThreshold,pRank[1],1, -1);
                }
...
//		String tagsByTaxaDirS = "M:/slice/";
//		String anchorFileS = "M:/pav/imp" + "/Zea20120110_scv10mF8maf002_mgs_E1pLD5kpUn.imp95_1024.c+.hmp.txt";
//		String outfileDirS = "M:/Result/";
		String tagsByTaxaDirS = args[0];
		String anchorFileS = args[1] + "/Zea20120110_scv10mF8maf002_mgs_E1pLD5kpUn.imp95_1024.c+.hmp.txt";
        String tagPMapFileS = args[2];
		String outfileDirS = args[3];
		new TagAgainstAnchorLongTimePosBlock (tagsByTaxaDirS, anchorFileS, tagPMapFileS, outfileDirS);
	}
} 
