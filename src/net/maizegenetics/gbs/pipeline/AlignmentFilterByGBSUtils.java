...
public class AlignmentFilterByGBSUtils {
    ...
    public static IdGroup getFilteredIdGroupByName(IdGroup orig, String[] filter, boolean keepTaxaMatchFilter) {
        ...
        for (int i = 0; i < orig.getIdCount(); i++) {
            ...
            for (String s: filter) {
                if(orig.getIdentifier(i).getFullName().contains(s)) matchFlag=true;
            }
            if(matchFlag==keepTaxaMatchFilter) keepTaxa.add(orig.getIdentifier(i));
        }
        ...
        for(int j=0; j<a.getSequenceCount(); j++) {
            for (int i = 0; i < a.getSiteCount(); i++) {
                if(a.getBase(j, i)!=DataType.UNKNOWN_BYTE) {
                    counts[0][j]++;
                    if(isRefAltCoded){if(a.getBase(j, i)==hetAllele) counts[1][j]++;}
                    else {if(nuc.isHeterozygote(a.getBase(j, i))) counts[1][j]++;}
                }
            }
...
//                    counts[0][j]+"\t"+counts[1][j]);
    ...
    public static int[][] genotypicCountsBySite(Alignment a, boolean isRefAltCoded, boolean printToScreen) {
        ...
        IUPACNucleotides nuc=new IUPACNucleotides();
        if(printToScreen) System.out.println("Locus\tMAF\tTaxaWKnown\tHetNum\tHetRate\tMajorAllele\tMajorCnt\tMinorAllele\tMinorCnt\tGapCnt");
        for (int i = 0; i < a.getSiteCount(); i++) {
            ...
            for(int j=0; j<a.getSequenceCount(); j++) {
                if(a.getBase(j, i)!=DataType.UNKNOWN_BYTE) {
                    counts[0][i]++;
                    if(isRefAltCoded){if (a.getBase(j, i) == hetAllele) counts[1][i]++;}
                    else {if (nuc.isHeterozygote(a.getBase(j, i))) counts[1][i]++;}
                    if(a.getBase(j, i)==majorAllele) counts[2][i]++;
                    if(a.getBase(j, i)==minorAllele) counts[3][i]++;
                    if(a.getBase(j, i)==DataType.GAP_BYTE) counts[4][i]++;
                }
            }
...
//            System.out.printf("%d %d %d %g %g %g %g %g %n",i, hetCnt[0][i], hetCnt[1][i], propHets,theMAF, expHets, relHets, obsF);
            if((obsF>minF)&&(hetCnt[0][i]>=minCount)&&(theMAF>=minMAF)&&(theMAF<=maxMAF)) {goodSites.add(i);}
        }
 ...
 //        if(theMAF<0.0001) System.out.printf("%d %s %d %d %d %g %n",i,a.getSNPID(i),hetCnt[3][i],hetCnt[1][i],hetCnt[0][i], theMAF);
    ...
    public static double getErrorRateForDuplicatedTaxa(Alignment a, boolean ignoreHets, boolean random, boolean printToScreen) {
        ...
        if(printToScreen) System.out.println("Entry1 Entry2 SNPDiff SNPsCompared PropDiff");
        for (Map.Entry<String,Integer> entry : sortedIds.entrySet()) {
            if(priorEntry.getKey().split(":")[0].equals(entry.getKey().split(":")[0])) {
                ...
                for (int i = 0; i < a.getSiteCount(); i++) {
                    byte pB=DataType.UNKNOWN_BYTE;
                    if(random) {
                        int t=(int)Math.round(Math.random()*(a.getSequenceCount()-1));
                        pB=a.getBase(t,i);
                    }
                    else {pB = a.getBase(priorEntry.getValue(), i);}
                    byte cB=a.getBase(entry.getValue(),i);
                    if(ignoreHets&&nuc.isHeterozygote(pB)) continue;
                    if(ignoreHets&&nuc.isHeterozygote(cB)) continue;
                    if((pB!=DataType.UNKNOWN_BYTE)&&(cB!=DataType.UNKNOWN_BYTE)) {
                        cntTotalTaxa++;
                        if(pB!=cB) cntDiffTaxa++;
                    }
                }
                if(printToScreen) System.out.printf("%s %s %d %d %g %n",priorEntry.getKey(), entry.getKey(), cntDiffTaxa, cntTotalTaxa, (double)cntDiffTaxa/(double)cntTotalTaxa);
                cntDiff+=cntDiffTaxa;
        ...
        if(random) {System.out.print("RandomDups ");} else {System.out.print("RealDups ");}
        if(ignoreHets) {System.out.print("ignoreHets ");} else {System.out.print("testHets ");}
    ...
    public static int[] getGoodSitesByLD(Alignment a, double minR2, double minBonferroniP, int minPosDist, int windowSize,
        ...
        for (int i = 0; i < a.getSiteCount(); i++) {
            ...
            for(int j=i-(windowSize/2); j<i+(windowSize/2); j++) {
                ...
                if(Math.abs(a.getPositionInLocus(i)-a.getPositionInLocus(j))<minPosDist) continue;
                cntInformative++;
            ...
            if(i%1000==0) System.out.printf(a.getSNPID(i)+" %d %g %g %g %n",cntInformative, obsMaxR2, obsMinP, bonP);
 //           if(obsMaxR2<minR2) System.out.println(i+" "+cntInformative+" "+obsMaxR2+" "+obsMinP);

            if(((obsMaxR2>=minR2)&&(bonP<=minBonferroniP))||(keepUnproven&&(cntInformative==0))) goodSites.add(i);
        }
        goodSites.trimToSize();
        return goodSites.elements();
    }
