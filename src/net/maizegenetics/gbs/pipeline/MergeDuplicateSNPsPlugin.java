...
public class MergeDuplicateSNPsPlugin extends AbstractPlugin {
    ...
    private void printUsage() {
        myLogger.info(
            "\n\nThe MergeDuplicateSNPsPlugin accepts the following parameters:\n"
    ...
    public void setParameters(String[] args) {
        ...
        if (myArgsEngine.getBoolean("-misMat"))       maxMisMat = Double.parseDouble(myArgsEngine.getString("-misMat"));
        ...
        if (myArgsEngine.getBoolean("-callHets"))     callHets = true;
        if (myArgsEngine.getBoolean("-kpUnmergDups")) kpUnmergDups = true;
    ...
    public DataSet performFunction(DataSet input) {
        ...
        for (int chr = startChr; chr <= endChr; chr++) {
            ...
            MutableSimpleAlignment msa = new MutableSimpleAlignment(a.getIdGroup(), a.getSiteCount(), a.getLoci());
            ...
            if (!kpUnmergDups) { deleteRemainingDuplicates(msa); }
            ExportUtils.writeToHapmap(msa, false, outfile, '\t');
        }
        return null;
    }

    private void processSNPsWithSamePosition(Integer[] samePos, Alignment a, int chr, int currentPos, MutableSimpleAlignment msa) {
        ...
        for (int s1=0; s1<samePos.length-1; ++s1) {
            if (finished[s1]) continue;  // s1 has already been merged with a previous site
            ...
            if (currAlleles[0]==IUPACNucleotides.GAP_BYTE || currAlleles[1]==IUPACNucleotides.GAP_BYTE) { 
            ...
            for (int s2=s1+1; s2<samePos.length; ++s2) {
                if (finished[s2]) continue;  // s2 has already been merged with a previous site (perhaps with different alleles)
                ...
                if (newAlleles[0]==IUPACNucleotides.GAP_BYTE || newAlleles[1]==IUPACNucleotides.GAP_BYTE) continue;
                ...
                if (Arrays.equals(currAlleles, newAlleles)) {
                    ...
                    for (int t = 0; t < a.getSequenceCount(); ++t) {                        
                        ...
                        if(currMerge[t]!=IUPACNucleotides.UNKNOWN_BYTE && geno2!=IUPACNucleotides.UNKNOWN_BYTE) {
                            if (!usePedigree || useTaxaForCompare[t]) ++nCompare;
                            if (currMerge[t]!=geno2) {
                                if (!usePedigree || useTaxaForCompare[t]) ++nMismatch;
                                possibleMerge[t] = callHets ? resolveHet(currMerge[t],geno2,hetKey) : IUPACNucleotides.UNKNOWN_BYTE;
                        ...
                        } else if (currMerge[t] == IUPACNucleotides.UNKNOWN_BYTE) {
                            possibleMerge[t] = geno2;
                        } else if (geno2 == IUPACNucleotides.UNKNOWN_BYTE) {
                            possibleMerge[t] = currMerge[t];
                        }
                    }
    ...
    private void addSiteToMutableAlignment(int chromosome, int position, byte[] genos, MutableSimpleAlignment theMSA) {
    ...
    private void deleteRemainingDuplicates(MutableSimpleAlignment theMSA) {
        ArrayList<Integer> samePosAL = new ArrayList<Integer>();
            ...
            if (newPos == currentPos) {   
                ...
                if (samePosAL.size() > 1) {
                    ...
                    for (int i = 0; i < samePos.length; ++i) {
                        ...
                        if (theMSA.getMajorAllele(samePos[i].intValue())!=IUPACNucleotides.GAP_BYTE 
                                && theMSA.getMinorAllele(samePos[i].intValue())!=IUPACNucleotides.GAP_BYTE) { 
    ...
    private static byte resolveHet(byte geno1, byte geno2, TreeMap<Integer,Byte> hetKey) {
        ...
        if (geno1 == IUPACNucleotides.GAP_BYTE && geno2 == IUPACNucleotides.GAP_BYTE) {
            return IUPACNucleotides.GAP_BYTE;
        }
        if (geno1 == IUPACNucleotides.GAP_BYTE || geno2 == IUPACNucleotides.GAP_BYTE) {
            return (byte) '0';
        }
        if (geno1 == (byte) '0' || geno2 == (byte) '0') {
            return (byte) '0';
        }
