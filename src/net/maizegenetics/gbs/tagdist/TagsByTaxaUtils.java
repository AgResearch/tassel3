   ...
   private static class TBTRecord {
        ...
        public byte[] taxonDist(){
            ...
            if(inputFormat==TagsByTaxa.FilePacking.Bit){
                ...
                for (int i= 0; i < currBitSet.capacity(); i++) {
                    if(currBitSet.fastGet(i)){ result[i]++; }
                }
    ...
    private static void writeRecord(TBTRecord currRecord){
        switch(outputFormat){
            case Bit:
                try {
                    for (Long i: currRecord.sequence){outputStream.writeLong(i);}
                    outputStream.write(currRecord.tagLength);
                    for (Long i: currRecord.bitDistribution){outputStream.writeLong(i);}
                } catch (Exception e) {
            ...
            case Text:
                ...
                try { writer.write(outputLine);
                } catch (Exception e){System.out.println("Caught Exception while writing TBT text record: "+e);}
                break;
        ...
        switch(inputFormat){
            ...
            case Text:
                ...
                try {
                    ...
                    for (int i= 0; i < numTaxa; i++) {
                        if(currLine[i+2].equals("1")){bitset.fastSet(i);}
                    }
        ...
        switch(outputFormat){
            ...
            case Byte:
                try {
                    ...
                    for(String name:taxaNames){outputStream.writeUTF(name);}
                } catch (Exception e) {
            ...
            case Text:
                try{
                    ...
                    for(String name:taxaNames){ writer.write(name+"\t");}
                    writer.write("\n");
                } catch (Exception e) {
...
//        }
    ...
    public static void filterByTOPM(String inputFileName, TagsOnPhysicalMap topm) {
        ...
        for (int i= 0; i < unfilteredLines; i++) {
           ...
           if(unfilteredLines%1000000==0) System.out.println("Counted "+(float)filteredLines/1000000+" million records.");
        }
        closeIOStreams();
        ...
        for (int i= 0; i < unfilteredLines; i++) {
           ...
           if(topm.getTagIndex(currRecord.sequence)>0){
               ...
               writtenLines++;
               if(writtenLines%1000000==0) System.out.println("Wrote "+(float)writtenLines/1000000+" million records.");
            }
        }
    ...
    public static void positions(String inputFileName, TagsByTaxa.FilePacking format, TagsOnPhysicalMap topm, String[] taxa){
        ...
        for (int i= 0; i < taxaNames.length; i++) {
            for(String taxon: taxa){
                if(taxaNames[i].equals(taxon)){columnsTracked.add(i);}
            }
        }
    ...
    private static void openIOStreams(String inputFileName, TagsByTaxa.FilePacking inputFormat, String outputFileName, TagsByTaxa.FilePacking outputFormat){
        ...
        if(outputFileName==null){outputFileName=inputFileName+".output";}
        else{TagsByTaxaUtils.outputFileName = outputFileName;}
    ...
    private static void closeIOStreams() {
        try {
            switch(inputFormat){
               ...
               case Bit:  inputStream.close(); break;
               case Text: reader.close(); break;
            }
            
            switch(outputFormat){
                case Byte: //Fall through
               case Bit: outputStream.close(); break;
               case Text: writer.close(); break;
            }
    ...
    public static HashMap<String, Integer> sumCounts(String inputFileName, TagsByTaxa.FilePacking format, boolean progressIndication){
        ...
        int[] tagCount = new int[numTaxa];
            ...
            for (int i= 0; i < numTags; i++) {
                if ( progressIndication && (i%100000==1) ) System.out.println("Finished reading " + i + "tags...");
                readRecord();
                ...
                if(format==TagsByTaxa.FilePacking.Bit){
                    ...
                    for(int j= 0; j < numTaxa; j++){
                        if(bitset.fastGet(j)){tagCount[j]++;}
                    }
...
//           }
    ...
    public static void sparsity(String inputFileName, TagsByTaxa.FilePacking format){
        ...
        for (int i = 0; i < numTags; i++) {
            readRecord();
            if(i%1000000==0) System.out.println("Read "+i/1000000+" million tags.");
    ...
    public static void printCoverage(String inputFileName, TagsByTaxa.FilePacking format, boolean itemized){
        ...
        if(itemized) System.out.println("Total "+taxaNames.length+" taxa."+'\n'+"sequence"+'\t'+"taxon coverage");
        for (int i = 0; i < numTags; i++) {
            readRecord();
            ...
            taxonCoverage[bin]++;
            if(itemized) System.out.println(BaseEncoder.getSequenceFromLong(currRecord.sequence)+'\t'+coverage);
            ...
            for (int j= 0; j < tagsCovered.length; j++) {
                if(currCounts[j] > 0){ tagsCovered[j]++; }else{ tagsUncovered[j]++;}
            }
        }

        //Summarize taxon coverage
        float total=0;
        if(!itemized) System.out.println("Taxon Coverage:");
        for(int count: taxonCoverage) total += ((float)count);
        if(!itemized){ 
        ...
        if(itemized) System.out.println("Total "+numTags+" tags."+'\n'+"taxon"+'\t'+"tag coverage"+'\t'+"fraction of total");
        for (int i= 0; i < taxaNames.length; i++) {
            float coverage=((float)tagsCovered[i])/((float)numTags);
            int bin=((int)(coverage*10));
            tagCoverage[bin]++;
            if(itemized) System.out.println(taxaNames[i]+'\t'+tagsCovered[i]+'\t'+coverage);
        }

       //Summarize tag coverage
       if(!itemized) System.out.println("Tag Coverage:");
       if(!itemized){
    ...
    public static void mergeTaxaByName(String inputFileName, String outputFileName, TagsByTaxa.FilePacking format, boolean caseSensitive){
        ...
        for (int i= 0; i < numTags; i++) {
            ...
            for (int j= 0; j < oldBitset.capacity(); j++) {
                if (oldBitset.fastGet(j)) newBitset.fastSet(newColumn[j]);
            }
