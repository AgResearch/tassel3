...
public abstract class AbstractTagsByTaxa extends AbstractTags implements TagsByTaxa{
    ...
    public OpenBitSet getTaxaReadBitsForTag(int readIndex) {
        ...
        for (int i = 0; i < getTaxaCount(); i++) {
            if(getReadCountForTagTaxon(readIndex,i)>0) r.fastSet(i);
        }
        return r;
    }
    ...
    public int getNumberOfTaxaWithTag(int readIndex) {   // how many taxa was a given read seen in?
        int nTaxaWData = 0;
        for (int i = 0; i < getTaxaCount(); i++) {
            if(getReadCountForTagTaxon(readIndex,i)>0) nTaxaWData++;
        }
        return nTaxaWData;
    }
   ...
   public void readDistFile(File inFile, FilePacking numberType) {
        ...
        switch (numberType) {
                case Text: readTextDistFile(inFile); break;
                case Bit: readBitDistFile(inFile); break;
                default: readByteShortDistFile(inFile, numberType); break;
            }
    }
...
//                        System.out.print("0\t");
    ...
    public void writeDistFile(File outFile, FilePacking numberType, int minCount) {
        ...
        try {
            ...
            switch (numberType) {
                case Text: hapsOutput = writeTextDistFile(fw,outReads, minCount); break;
                case Bit: hapsOutput = writeBitDistFile(fw,outReads, minCount); break;
                default: hapsOutput = writeByteShortDistFile(fw, numberType, outReads, minCount); break;
            }
            fw.flush();
            fw.close();
    ...
    private int writeTextDistFile(DataOutputStream fw, int outReads, int minCount) {
        ...
        try{
            ...
            for (int i = 0; i < tags[0].length; i++) {
                if (minCount > 0 && getReadCount(i) < minCount) continue;
    ...
    private int writeByteShortDistFile(DataOutputStream fw, FilePacking countType, int outReads, int minCount) {
        ...
        try{
            ...
            for (int i = 0; i < tags[0].length; i++) {
                if (minCount > 0 && getReadCount(i) < minCount) continue;
                for (int j = 0; j < tagLengthInLong; j++) fw.writeLong(tags[j][i]);
    ...
    private int writeBitDistFile(DataOutputStream fw, int outReads, int minCount) {
        ...
        try{
            ...
            for (int i = 0; i < tags[0].length; i++) {
                if (minCount > 0 && getReadCount(i) < minCount) continue;  //Skip tags whose count is lower than a cutoff
                for (int j = 0; j < tagLengthInLong; j++) fw.writeLong(tags[j][i]);
                fw.writeByte(tagLength[i]);
                obs=new OpenBitSet(taxaNum);
                for (int t = 0; t < taxaNum; t++) {
                    if(getReadCountForTagTaxon(i, t)>0) obs.set(t);
                }
    ...
    private int readsWCountsGreaterThanMin(int minCount) {
        if (minCount < 1) return getTagCount();
        int sum = 0;
