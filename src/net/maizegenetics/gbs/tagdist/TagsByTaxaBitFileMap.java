...
public class TagsByTaxaBitFileMap extends AbstractTagsByTaxa{
    ...
    public TagsByTaxaBitFileMap(String outFile, String[] taxaNames, Tags theTags) {
        ...
        try{
            ...
            for (int i = 0; i < theTags.getTagCount(); i++) {
                for (int j = 0; j < tagLengthInLong; j++) fw.writeLong(theTags.getTag(i)[j]);
        ...
        try {
		    ...
		    tags[j][i] = bb.getLong();
                ...
                for (int j= 0; j < taxaNum; j++) {
                    currTagCount = bb.get();
                    if(currTagCount>0){presenceCountByTaxon[j]++;}
                }
                hapsOutput++;
            }
    ...
    public void setReadCountForTagTaxon(int tagIndex, int taxaIndex, int value) {
        if(value==getReadCountForTagTaxon(tagIndex, taxaIndex)) return;
        if(rowSetMethod) {setReadCountForTagTaxonRow(tagIndex, taxaIndex, value);}
        else {setReadCountForTagTaxonPoint(tagIndex, taxaIndex, value);}
    }
    ...
    private void setReadCountForTagTaxonRow(int tagIndex, int taxaIndex, int value) {
        ...
        if(value==0) {bufferedTagDist.fastClear(taxaIndex);}
            else {bufferedTagDist.fastSet(taxaIndex);}
        bufferChanged=true;
    }

    private void setReadCountForTagTaxonPoint(int tagIndex, int taxaIndex, int value) {
        try{int wordNum = taxaIndex >> 6;
...
//            for (int t = 0; t < obsInLong.length; t++) theRAF.writeLong(obsInLong[t]);
            ...
            for (int t = 0; t < obsInLong.length; t++) bb.putLong(obsInLong[t]);
