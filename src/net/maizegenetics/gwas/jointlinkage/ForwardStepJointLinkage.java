...
public class ForwardStepJointLinkage implements Callable<ForwardStepResult> {
	...
	public ForwardStepResult call() throws Exception {
		...
		double[] baseModelSSdf = plm.getLinearModel().getFullModelSSdf();
		...
		double modeldf = plm.getModeldf() + baseModelSSdf[1];
		double nobs = plm.getErrordf() + modeldf;
		
		//model df is model corrected for mean. Add 1 for mean and 1 for errorvar
		result.bestAICc = StepwiseModelFitterForNAMJointLinkage.aicc(errorvar, nobs, modeldf + 1);
		return result;
	}
