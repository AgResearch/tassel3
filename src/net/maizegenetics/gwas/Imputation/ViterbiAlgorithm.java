...
public class ViterbiAlgorithm {
	...
	TransitionMatrix myTransitionMatrix;
	ConditionalProbability probObservationGivenState;
	byte[] obs;
	byte[][] history;
	double[] distance;
	...
	public ViterbiAlgorithm(byte[] observations, TransitionMatrix transitionMatrix, ConditionalProbability obsGivenTrue, double[] pTrue) {
		obs = observations;
	...
	public void initialize() {
		for (int i = 0; i < numberOfStates; i++) {
			try{distance[i] = probObservationGivenState.lnProbabilityAgivenB(obs[0], i, 0) + probTrueStates[i];} catch(Exception e) {
				e.printStackTrace();
			}
		}
	}
	...
	public void updateDistanceAndHistory(int node) {
		...
		myTransitionMatrix.setObservation(node);
		for (int i = 0; i < numberOfStates; i++) {
			for (int j = 0; j < numberOfStates; j++) {
				candidateDistance[i][j] = distance[i] + myTransitionMatrix.getLnTransitionProbability(i, j, node) + probObservationGivenState.lnProbabilityAgivenB(obs[node], j, node);
			}
		}
